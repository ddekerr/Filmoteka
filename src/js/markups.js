...
export function createFilmsGallery(items) {
    ...
    <li class="film">
      <a class="film__link link" href="">
        <div class="film__image-container">
          <img class="film__image" src="https://www.vaureal.fr/sites/vaureal/files/styles/_site_contenu_image_principale/public/image/2022-03/Solidarit%C3%A9%20ukraine.jpg?itok=Gcqq2tD3" alt="${item.original_title}" load="lazy"/>
        </div>
  ...
  `}
    ...
    <li class="film">
      <a class="film__link link" href="">
        <div class="film__image-container">
          <img class="film__image" src="${config.postersUrl + config.postersSize + item.poster_path}" alt="${item.original_title}" load="lazy"/>
        </div>
        <h2 class="film__name title">${item.title}</h2>
        <div class="film__description">
          <p class="film__genre">${genersForFilmCard(item.genre_ids)}</p>
